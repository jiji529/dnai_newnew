	...
	$.ajax({
        ...
        success : function(data) {
        	...
        	if(eval(data).length == 0){	
            	...
            	$(".member_total_info").html('<li>프리미엄 서비스 출시 이후 고객님의 계정에 어제까지 뷰어에 등록된 기사(가판 제외)를 <strong>TF-IDF 알고리즘</strong>을 활용하여 분석했습니다.<a href="#" onclick = "showTFIDFPopup()">TF-IDF란?</a></li>'+
...
function show_member_score_infoText(){
	var msg = "프리미엄 서비스 출시 이후 고객님의 프리미엄 계정에 등록된 기사(가판 제외)를 분석했습니다. 워드 클라우드 단어를 누르면 해당 단어의 점수 추이선이 표시 됩니다.";
	alert(msg);
}

function show_member_score_period_infoText(){
	var msg = "주간은 최근 7일, 월간은 최근 30일, 분기는 최근 90일에 고객님의 프리미엄 계정에 등록된 기사(가판 제외)를 분석했습니다";
	alert(msg);
}
...
function table_img_download(div_id){
	var word_table_document_object = document.getElementById(div_id);
	word_table_document_object.style.fontFeatureSettings = '"smcp", "zero"';
	var word_table = $("#"+div_id);
	word_table.find(".desc_info > span").each(function() {
		$(this).contents().unwrap().wrap("<text></text>");
		this.style.fontFeatureSettings = '"smcp", "zero"';
	});
	word_table.find(".desc_info > a").each(function() {
		$(this).hide();
	});
	var width = word_table.outerWidth()+10;
	var height = word_table.outerHeight();
	
	html2canvas(word_table_document_object, {width : width, height: height, scrollY: -window.scrollY, scale : 1.5, useCORS : true}).then(function (canvas) {
        var img = canvas.toDataURL('image/png');
        var file_name = "word_table.png";
        downloadURI(img, file_name, width, height);
   });
	word_table.find(".desc_info > a").each(function() {
		$(this).removeAttr("style");
	});
	word_table.find(".desc_info > text").each(function() {
		$(this).contents().unwrap().wrap("<span></span>");
		this.style.fontFeatureSettings = '';
	});
}
function total_table_img_download() {
	var div_id = "word-table";
	//table_img_download(div_id);
}
function member_table_img_download() {
	var div_id = "member_word-table";
	//table_img_download(div_id);
}
function table_img_download_sentiment() {
	var div_id = "sentiment_member_word-table";
	//table_img_download(div_id);
}